...
# =============================================================================
# CORRECCIÓN: Normalizador de Acciones
# =============================================================================

set -e

echo "=============================================="
echo "  CORRECCIÓN: Normalizador de Acciones"
echo "=============================================="
echo ""

# -----------------------------------------------------------------------------
# 1. Corregir src/motor/normalizador.py
# -----------------------------------------------------------------------------
echo "→ Corrigiendo src/motor/normalizador.py..."
...
class NormalizadorAcciones:
    ...
    ACCIONES_GENERICAS = {
        'dash': ['dash', 'carrera', 'sprint', 'correr rápido', 'correr rapido'],
        'dodge': ['dodge', 'esquivar', 'esquiva', 'evadir', 'esquivo'],
        'disengage': ['disengage', 'desenganche', 'retirada', 'retirarse', 'retirarme'],
        'help': ['help', 'ayudar', 'ayuda', 'asistir', 'ayudo'],
        'hide': ['hide', 'esconder', 'esconderse', 'esconderme', 'ocultar', 'ocultarme', 'me escondo'],
        'search': ['search', 'buscar', 'registrar'],
        'ready': ['ready', 'preparar', 'preparar acción', 'preparar accion', 'preparo']
    }
    ...
    def normalizar(self, texto: str, 
        ...
        # Paso 2: Detectar intención (orden de prioridad importante)
    ...
    def _detectar_intencion(self, texto: str, contexto: ContextoEscena) -> Tuple[TipoAccionNorm, float]:
        ...
        # También buscar en el compendio
        ...
        # 3. Tercero: detectar por patrones de verbo
        
        # Conjuros (si dice "conjuro", "hechizo", "magia")
        if re.search(r'\b(conjuro|hechizo|magia)\b', texto):
            return TipoAccionNorm.CONJURO, 0.85
        
        # Movimiento
        if re.search(r'\b(muevo|moverme|camino|caminar|corro|correr|acerco|acercarme|alejo|alejarme|desplazo|desplazarme|voy|ir)\b', texto):
            return TipoAccionNorm.MOVIMIENTO, 0.85
        ...
        # Verbos que implican habilidades
        if re.search(r'\b(escucho|escuchar|examino|examinar|investigo|investigar)\b', texto):
            return TipoAccionNorm.HABILIDAD, 0.8
        
        # Objetos
        if re.search(r'\b(uso|usar|bebo|beber|tomo|tomar)\b.*\b(poci[oó]n|objeto|item)\b', texto):
            return TipoAccionNorm.OBJETO, 0.85
        if re.search(r'\bpoci[oó]n\b', texto):
            return TipoAccionNorm.OBJETO, 0.8
        
        # Ataque (más general, va al final)
        if re.search(r'\b(ataco|atacar|ataque|golpeo|golpear|pego|pegar|disparo|disparar|lanzo|corto|cortar|apuñalo|hiero|herir)\b', texto):
            return TipoAccionNorm.ATAQUE, 0.85
    ...
    def _normalizar_movimiento(self, texto: str,
        ...
        # Buscar distancia
        # Patrones: "20 pies", "5 metros", "6m", "30ft", "20pies"
        ...
        if match_pies:
            ...
            datos["distancia_pies"] = int(metros * 3.28)
        elif match_casillas:
            casillas = int(match_casillas.group(1))
            datos["distancia_pies"] = casillas * 5
        else:
        ...
        for patron in destinos_comunes:
            ...
            if match:
                grupos = match.groups()
                datos["destino"] = grupos[-1] if grupos else None
                break
    ...
    def _normalizar_habilidad(self, texto: str,
        ...
        if not datos["habilidad"]:
            # Inferir por verbos
            if re.search(r'\b(escucho|escuchar|oigo|oir)\b', texto):
                datos["habilidad"] = "percepcion"
            elif re.search(r'\b(busco|buscar|miro|mirar)\b', texto):
                datos["habilidad"] = "percepcion"
            elif re.search(r'\b(investigo|investigar|examino|examinar)\b', texto):
                datos["habilidad"] = "investigacion"
            elif re.search(r'\b(salto|saltar|trepo|trepar|escalo|escalar)\b', texto):
                datos["habilidad"] = "atletismo"
            elif re.search(r'\b(ruedo|rodar|esquivo)\b', texto):
                datos["habilidad"] = "acrobacias"
            elif re.search(r'\b(miento|mentir|engaño|engañar)\b', texto):
                datos["habilidad"] = "engaño"
            elif re.search(r'\b(convenzo|convencer|persuado|persuadir)\b', texto):
                datos["habilidad"] = "persuasion"
            elif re.search(r'\b(intimido|intimidar|amenazo|amenazar)\b', texto):
    ...
    def _buscar_arma_en_texto(self, texto: str,
        ...
        """Busca un arma mencionada en el texto. Retorna compendio_ref."""
        ...
        for arma in contexto.armas_disponibles:
            ...
            if nombre_lower and nombre_lower in texto.lower():
                # Retornar compendio_ref, no el id de instancia
    ...
    def _buscar_objetivo_en_texto(self, texto: str,
        ...
        # Buscar por palabras clave del nombre (ej: "arquero" en "Goblin arquero")
        for enemigo in contexto.enemigos_vivos:
        ...
        # Buscar tipo de criatura (compendio_ref)
        for enemigo in contexto.enemigos_vivos:
    ...
    def _resolver_ambiguedades(self, resultado: AccionNormalizada,
        ...
        if "arma_id" in faltantes and resultado.tipo == TipoAccionNorm.ATAQUE:
            if contexto.arma_principal:
                # Usar compendio_ref, no el id de instancia
                ref = contexto.arma_principal.get("compendio_ref") or contexto.arma_principal.get("id")
                datos["arma_id"] = ref
                faltantes.remove("arma_id")
...
echo "   ✓ normalizador.py corregido"

# -----------------------------------------------------------------------------
# 2. Actualizar tests con expectativas correctas
# -----------------------------------------------------------------------------
echo "→ Actualizando tests/test_normalizador.py..."
...
echo "   ✓ test_normalizador.py actualizado"
...
echo ""
echo "Cambios:"
echo "  1. Orden de prioridad en detección de intención"
echo "  2. Patrones de regex corregidos"
echo "  3. Búsqueda de armas retorna compendio_ref"
echo "  4. Búsqueda de objetivos por palabras clave"
echo "  5. Tests ajustados a expectativas realistas"
echo ""
echo "Siguiente paso:"
